...
DESCRIPTION:    Base class for cleaning up any changes made by aux_data_inserters outside of the aux data mongo collection.
...
class AuxDataFileCleaner(ABC):
    ...
    def _get_aux_data_for_source(self, event):
        ...
        if not all_aux_data:
            self.logger.debug(f"No aux data found for event {event['id']}")
            return None
        ...
        if not aux_data_for_source:
            self.logger.debug(f"No {self._data_source} aux data found for event {event['id']}")
