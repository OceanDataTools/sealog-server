...
DESCRIPTION:    This service listens for new events submitted to Sealog, create
                ...
                long as the data is availble from the InfluxDB.  In the event
...
from misc.coriolix.aux_data_record_builder import SealogCORIOLIXAuxDataRecordBuilder
...
if __name__ == '__main__':
    ...
    aux_data_builder_list = list(map(lambda config: SealogCORIOLIXAuxDataRecordBuilder(CORIOLIX_API_URL, config), aux_data_configs))
    ...
    while True:
        ...
        try:
            ...
            asyncio.get_event_loop().run_until_complete(insert_aux_data_from_ws(aux_data_builder_list, parsed_args.dry_run))
