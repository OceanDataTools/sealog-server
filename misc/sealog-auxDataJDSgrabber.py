...
#          and creates/associates an aux_data record contain current
#          vehicle position/attitude data to the newly created event.
#          However this will only happen if the timestamp of the event
#          is within 120 seconds from the current time. 
#
#   Usage: Type python3 sealog-auxDataJDSgrabber.py to start the service.
#
...
excludeSet = ('SuliusCam')

clientWSID = 'jasonJDSgrabber'
        ...
        async with websockets.connect('ws://' + serverIP + ':' + serverWSPort) as websocket:
            ...
            while(True):
                ...
                if eventObj['type'] and eventObj['type'] == 'ping':
                    ...
                    eventTS = datetime.strptime(eventObj['message']['ts'], '%Y-%m-%dT%H:%M:%S.%fZ')
                    if (nowTS - eventTS).seconds <= ignoreAfter:
                        record = collection.find_one({'label':'JDS'})
                        
                        auxData = auxDataTemplate

                        auxData['event_id'] = eventObj['message']['id']
                        auxData['data_source'] = "vehicleRealtimeNavData"
                        auxData['data_array'] = []
