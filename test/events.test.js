...
describe('Events API', () => {
  ...
  describe('DELETE /events/{id}', () => {
    ...
    it('leaves aux_data for the aux_data_manager to clean up via WebSocket', async () => {
      await server.inject({
        method: 'DELETE',
        url: `/sealog-server/api/v1/events/${testEvent._id}`,
        headers: { Authorization: 'Bearer ' + loggerJwt }
      ...
      // Aux data is NOT deleted directly — the server publishes a deleteEvents WS
      // message and aux_data_managers handle deletion asynchronously.
      const auxData = await db.collection(eventAuxDataTable).findOne({ _id: testAuxData._id });
      expect(auxData).to.exist();
    });
